...
shinyServer(function(input, output, session){ 
  ...
  observeEvent(input$load_config,{
    tryCatch({
      ...
      NumberSpecies = length(MySpeciesTraits$nomEspece)
      if(NumberSpecies == 1){
        height = 50
      }else if(NumberSpecies> 1 && NumberSpecies<= 5){
        height = 150
      ...
      if(!is.null(MySpeciesTraits)){
        output$data_logging <-rhandsontable::renderRHandsontable({
          rhandsontable::rhandsontable(MySpeciesTraits, colHeaders = c("Nom", "Code_espece", "D.M.E", "D.M.A", "Taux_de_prelevement", "Coefficient_de_recolement", "Tarif_de_cubage", "densite"), selectCallback = TRUE, useTypes = TRUE, width = "100%", height = height) %>%
  ...
  updateUIInputsValues<- function(Parcelles, Campagnes, ClassesDiam, Species, AnneeExploitation){
    ...
    updateSelectizeInput(session, "groupe_espece_indicateur", choices = Species,
                         selected = NULL,  server = TRUE)
    updateSelectizeInput(session, "logging_species", choices = Species,
                         selected = NULL,  server = TRUE)
  ...
  loadTableDataSpeciesTraits <- function(SpeciesTraits){
    output$data_logging <-rhandsontable::renderRHandsontable({
      ...
      rhandsontable::rhandsontable(data_log, colHeaders = c("Nom", "Code_espece", "D.M.E", "D.M.A", "Taux_de_prelevement", "Coefficient_de_recolement", "Tarif_de_cubage", "densite"), selectCallback = TRUE, useTypes = TRUE, width = "100%", height = height) %>%
        hot_table(highlightCol = TRUE, highlightRow = TRUE)%>% 
  ...
  loadTableDataDamagesVector <- function(MyClassesDiam){
    output$vector_damage <-rhandsontable::renderRHandsontable({
      ...
      StrEval2 = paste0(StrEval2, '"[', ClassesDiam[length(MyClassesDiam)],', Inf[")')
      eval(parse(text = StrEval2))
      StrEval3 = paste0(StrEval3, paste0(colwidths,')'))
      eval(parse(text = StrEval3))
  ...
  loadTableNumberParDiameterClassVector <- function(MyClassesDiam){
    output$vector_number_per_diameter_class <-rhandsontable::renderRHandsontable({
      ...
      StrEval2="ColHeaders = c("
      StrEval3="colWidths = c("
      colwidths =1000/length(MyClassesDiam) 
      for(iter in 1:(length(MyClassesDiam)-1)){
        StrEval1 = paste0(StrEval1, 'classe', iter,'=c("NA"), ')
        StrEval2 = paste0(StrEval2, '"[', MyClassesDiam[iter],', ', MyClassesDiam[iter+1], '[", ')
        StrEval3 = paste0(StrEval3, paste0(colwidths,', '))
  ...
  observeEvent(input$filter_logging_species,{
    tryCatch({
      ...
      #DF  <-  hot_to_r(input$data_logging)
      #browser()
      #FilterSpeciesTraits = subset(DF, codeEspece%in%loggingSpecies)
      #browser()
      #if(!is.null(FilterSpeciesTraits)){
      #  output$data_logging <-rhandsontable::renderRHandsontable({
      #    rhandsontable::rhandsontable(FilterSpeciesTraits, colHeaders = c("Nom", "Code_espece", "D.M.E", "D.M.A", "Taux_de_prelevement", "Coefficient_de_recolement", "Tarif_de_cubage", "densite"), selectCallback = TRUE, useTypes = TRUE, width = "100%") %>%
      #      hot_cols(columnSorting = TRUE, colWidths= c(150, 100, 75, 75, 150, 175, 200, 75), manualColumnResize=TRUE, fixedColumnsLeft=1) %>%
      #      #hot_cols(columnSorting = TRUE, fixedColumnsLeft=1) %>%
      #      hot_col(col = "Nom", type = "autocomplete", source =FilterSpeciesTraits$Nom)
      # })
      #}
  ...
  observeEvent(input$load_dyn,{
    tryCatch({
      ...
      ParamsDyn = readRDS(Loadpath)
      if(!is.null(ParamsDyn)){
        TypeInventaire = ParamsDyn$Descriptif$TypeInventaire
        NewSpeciesTraits = ParamsDyn$SpeciesTraits
        if(!is.null(NewSpeciesTraits)){
      ...
      shinyjs::info("les paramètres de la dynamique ont été chargés avec succès")
    },error=function(e){
      shinyjs::show('load_dyn')
      shinyjs::hide('boxloader_FileDyn')
...
updateSelectizeInput(session, "groupe_espece_indicateur", choices = MBaikiFormatted$TraitData$Id.sp,
                     selected = NULL,  server = TRUE)
updateSelectizeInput(session, "logging_species", choices = MBaikiFormatted$TraitData$Id.sp,
                     selected = NULL,  server = TRUE)
