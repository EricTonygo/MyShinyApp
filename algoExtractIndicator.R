...
PlotMyIndicatorParcelle <-function(out.FCM,Outputs=NULL,Groups=NULL,Verif=NULL, MyClassesDiam=NULL, MyTimeInterval=NULL, MyIndicator=NULL, Lab.period="years"){
  #########################################
  ...
  Simulations=merge(Simulations,DataTraits,by="Nom.sp", all.x=F,all.y=F)
  ...
  if (is.null(Groups)) Groups=list(stand=as.factor(levels(Simulations$Nom.sp)))
  
  if (Verif){
    SimData=out.FCM$DataVerif
    if (out.FCM$agggregation){
      DataVerif=subset(SimData,Id.zone%in%out.FCM$Starting.plot & Id.campagne>=StartingDate)
      DataVerif$Id.zone=paste0(out.FCM$Starting.plot,collapse="-")
    }else{DataVerif=subset(SimData,Id.zone%in%(unique(Simulations$Id.zone))&Id.campagne>=StartingDate)}
    DataVerif=data.table(DataVerif,key="ClassesDiam,Nom.sp,Id.zone,Id.campagne")
    DataVerif[,Efft:=1]
    DataVerif=DataVerif[,list(Effectifs=sum(Efft)),by="Id.zone,ClassesDiam,Nom.sp,Id.campagne"]
    setkey(DataVerif,"ClassesDiam")
    setkey(DataVerif,"Nom.sp")
    DataVerif=merge(DataVerif,DataTraits,by="Nom.sp",all.x=F,all.y=F)
    DataVerif=merge(DataVerif,DataOutputs,by=c("Nom.sp","ClassesDiam"),all.x=F,all.y=F)
    if(!is.null(MyClassesDiam)){
    ...
    #DataVerif = DataVerif[Temps %in% MyTimeInterval]
  }
  
  Simulations$ClassesDiam=as.factor(Simulations$ClassesDiam)
  Myggplot = ggplot()
  names_species_title = NULL
  ...
  Mytitle=paste("Prediction de l'indicateur: ", MyIndicator$NomInd, " des especes: ",names_species_title)
  for (g in 1:length(Groups)){
    ...
    if(nrow(SimulationTmp) != 0){
      ...
      if(MyIndicator$NomInd == "Taux de reconstitution du stock"){
        ...
        ExpEvalSim = paste0("Indicateurs[,",MyIndicator$VarInd,"sR:=",MyIndicator$VarInd,"s*100/",MyIndicator$VarInd,"sInit]", collapse = '')
        eval(parse(text = ExpEvalSim))
        ExpEvalSim = paste0("Indicateurs",MyIndicator$VarInd,"=Indicateurs[,list(li",MyIndicator$VarInd,"s=quantile(",MyIndicator$VarInd,"sR,0.025,na.rm=T),med",MyIndicator$VarInd,"s=median(",MyIndicator$VarInd,"sR),ls",MyIndicator$VarInd,"s=quantile(",MyIndicator$VarInd,'sR,0.975,na.rm=T)),by="Temps"]', collapse = '')
        eval(parse(text = ExpEvalSim))
      }else{
    ...
    if (Verif){
      DataVerifTmp=subset(DataVerif,Nom.sp%in%Groups[[g]])
      if(nrow(DataVerifTmp) != 0){
        ...
        eval(parse(text = ExpEvalSim))
        if(MyIndicator$NomInd == "Taux de reconstitution du stock"){
          ExpEvalSim = paste0("tmp=subset(IndicateursVerif,select=c(Id.zone,", MyIndicator$VarInd,"s),Temps==min(Temps))", collapse = '')
          eval(parse(text = ExpEvalSim))
          ExpEvalSim = paste0('setnames(tmp,"',MyIndicator$VarInd,'s","',MyIndicator$VarInd,'sInit")', collapse = '')
          eval(parse(text = ExpEvalSim))
          IndicateursVerif=merge(IndicateursVerif,tmp,by="Id.zone",all.x=F,all.y=F)
          ExpEvalSim = paste0("IndicateursVerif[,",MyIndicator$VarInd,"sR:=",MyIndicator$VarInd,"s*100/",MyIndicator$VarInd,"sInit]", collapse = '')
          eval(parse(text = ExpEvalSim))
          ExpEvalSim = paste0("IndicateursVerif","=IndicateursVerif[,list(",MyIndicator$VarInd,"s=median(",MyIndicator$VarInd,'sR)),by="Temps"]', collapse = '')
          eval(parse(text = ExpEvalSim))
        }else{
        ...
        eval(parse(text =  ExpEvalSim))
        ExpEvalSim = paste0("Myggplot=Myggplot + geom_point(data=data, aes(x=temps, y=verif",MyIndicator$VarInd, ', colour="Verif',MyIndicator$VarInd,'"), size=5)', collapse = '')
        eval(parse(text =  ExpEvalSim))  
      }
    }
  }
