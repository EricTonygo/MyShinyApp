RecruitmentNNET <- function(DataFormatted,ParamFiles,criterion="BIC"){
  ...
  for (i in 1:10){
    ...
    NN.cur=nnet(modelRecrut,data=DataRecrut,size=8,linout=T,rang=0.023,decay=0.2,maxit=5000)
    if (NN.cur$value<score) {
                        NN=NN.cur
                        score=NN.cur$value
                              }
  }
  ...
  SimRecrut$IndexVarRecrut=NomsSTTC%in%VarRecrut
  ...
  SimRecrut$Func <-function(Eff.cur,SimRecrut){
    ...
    STTC=matrix(STTC[SimRecrut$IndexVarRecrut],nrow=1)
    colnames(STTC)=SimRecrut$VarRecrut
    NbRecrut=max(predict(SimRecrut$NN,STTC),0)

    weight=dnorm(SimRecrut$NbRecrut,mean=NbRecrut,sd=sqrt(NbRecrut))
