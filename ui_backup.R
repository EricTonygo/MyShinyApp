                  ...
                  tabBox( width = NULL,
                          ...
                          tabPanel(h5(strong("Paramètres de l'exploitation")),
                                   ...
                                   fluidRow(
                                     column(width= 10,
                                            selectizeInput(
                                              'logging_species', label = "Les espèces exploitées", multiple= TRUE, choices = c(221, 222, 223, 224, 225),
                                              options = list(create = FALSE), width = '100%'
                                            )
                                     ),
                                     column(width = 2, id="filter_logging_species_col",
                                            actionButton("filter_logging_species", strong("Filter"), icon = NULL, style="display: inline-block; color: #fff; background-color: #337ab7; border-color: #2e6da4;"),
                                            shinyjs::hidden(div(id ='boxloader_filter_logging_species', width ="100px",
                                                                div(id="img_loader_FLS", img(src= "trait_loader.gif", width= "100px")),
                                                                tags$style(type='text/css', "#img_loader_FLD { text-align: center;}")
                                                                #div(id="load_FLS_encours", strong("Traitement en cours ...")),
                                                                #tags$style(type='text/css', "#load_FLD_encours {text-align: center;}")
                                            )
                                            ),
                                            tags$style(type='text/css', "#filter_logging_species:hover { border-color:#979494 !important;}")
                                     ),
                                     tags$style(type='text/css', "#filter_logging_species_col {padding-top:2%}")
                                     
                                   ),
                                   shinyjs::hidden(
                                     div(id="bloc_simulation_type",
                                         br(),
                                         fluidRow(
                                           column(width =  6,
                                                  radioButtons("simulation_type", label = NULL,
                                                               choices = list("Simulation des Parcelles" = 1, "Simulation du Sentier" = 2), inline = TRUE)
                                           )
                                         )
                                     )
                                   ),
                                   ...
                                   shinyjs::hidden(
                                     div(id="bloc_simulation_type",
                                         ...
                                         shinyjs::hidden(
                                           div(id="bloc_number_per_diameter_class",
                                               fluidRow(
                                                 box(
                                                   h5(strong("Vecteur des effectifs par classe de diamètre")), width = "100%",
